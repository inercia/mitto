		...
		procMgr.SetRunRecorder(func(run processors.ProcessorRun) {
			if err := store.AppendEvent(sessionID, session.Event{
				...
				Data: session.ProcessorRunData{
					...
					Target:        run.Target,
					RunKind:       run.RunKind,
					RerunReason:   run.RerunReason,
					SkipReason:    run.SkipReason,
					PromptSeq:     run.PromptSeq,
				},
			}); err != nil {
...
func (sm *SessionManager) SetEventsManager(eventsManager EventsBroadcaster) {
	sm.mu.Lock()
	defer sm.mu.Unlock()
	if eventsManager == nil {
		sm.eventsManager = nil
		return
	}
...
func (sm *SessionManager) SetPromptResolver(resolver PromptResolver) {
	sm.mu.Lock()
	defer sm.mu.Unlock()
...
// Used by HTTP handlers in the web layer that need to resolve effective runner config without
// accessing unexported fields across package boundaries.
func (sm *SessionManager) GetGlobalRunnerInfo() (map[string]*config.WorkspaceRunnerConfig, *config.Config) {
	sm.mu.RLock()
	defer sm.mu.RUnlock()
	return sm.globalRestrictedRunners, sm.mittoConfig
}
...
// This bypasses all lifecycle logic and is intended only for unit tests that need a
// pre-seeded session without running the full ACP startup path.
func (sm *SessionManager) AddSessionForTest(bs *BackgroundSession) {
	sm.mu.Lock()
	sm.sessions[bs.GetSessionID()] = bs
	sm.mu.Unlock()
}
