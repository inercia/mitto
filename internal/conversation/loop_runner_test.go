...
// TestLoopRunner_RecordTasksFireOutcome_NoProgressLimitOptOut is the failing
// reproduction for mitto-ckaa: a supervisor-style onTasks loop that sets
// NoProgressLimit=0 (unlimited) on its persisted LoopPrompt expects the
// Layer 3 circuit breaker to never trip, even under many consecutive
// no-progress fires. On the current runner (which unconditionally consults
// the hardcoded tasksNoProgressLimit constant instead of the per-loop opt-out),
// the loop is auto-paused after 3 fires — this test fails until the runner
// starts honouring LoopPrompt.EffectiveNoProgressLimit().
func TestLoopRunner_RecordTasksFireOutcome_NoProgressLimitOptOut(t *testing.T) {
	store, err := session.NewStore(t.TempDir())
	if err != nil {
		t.Fatalf("NewStore() error = %v", err)
	}
	...
	// Configure the persisted LoopPrompt with the opt-out: NoProgressLimit=0
	// means unlimited (never trip the breaker) — the supervisor pattern.
	loop, err := ps.Get()
	if err != nil {
		t.Fatalf("Get() error = %v", err)
	}
	zero := 0
	loop.NoProgressLimit = &zero
	...
	// Fire many no-progress rounds — well past the default breaker threshold
	// (tasksNoProgressLimit = 3). With the opt-out honoured, the loop must
	// stay enabled throughout; with the current buggy runner it auto-pauses
	// on the 3rd fire.
	delta := &config.TasksDelta{Touched: []map[string]any{{"id": "mitto-1"}}}
	runner.recordTasksFireOutcome("s1", ps, delta) // seed
	const rounds = 20
	...
	if !got.Enabled {
		t.Errorf("loop with NoProgressLimit=0 (opt-out) should stay enabled after %d no-progress fires; got Enabled=false, StoppedReason=%q (mitto-ckaa)", rounds, got.StoppedReason)
	}
	if got.StoppedReason == session.StoppedReasonNoProgress {
		t.Errorf("loop with NoProgressLimit=0 (opt-out) should NOT be auto-paused by the no-progress circuit breaker; got StoppedReason=%q (mitto-ckaa)", got.StoppedReason)
	}
}

// TestLoopPrompt_EffectiveNoProgressLimit is the pure helper unit test for
// LoopPrompt.EffectiveNoProgressLimit — sibling of
// TestLoopPrompt_ShouldCoalesceDuringBusy_Default in internal/session. Kept
// here alongside the runner reproduction so both stay green together as the
// mitto-ckaa fix lands.
func TestLoopPrompt_EffectiveNoProgressLimit(t *testing.T) {
	p := &session.LoopPrompt{}
	if got := p.EffectiveNoProgressLimit(); got != 3 {
		t.Errorf("unset NoProgressLimit: got %d, want 3 (default)", got)
	}
	zero := 0
	p.NoProgressLimit = &zero
	if got := p.EffectiveNoProgressLimit(); got != 0 {
		t.Errorf("*0 NoProgressLimit: got %d, want 0 (unlimited)", got)
	}
	five := 5
	p.NoProgressLimit = &five
	if got := p.EffectiveNoProgressLimit(); got != 5 {
		t.Errorf("*5 NoProgressLimit: got %d, want 5 (custom)", got)
	}
}
