...
func NewManager(store *session.Store, catalog Catalog, credentials CredentialResolver, runner ManagedLoopTriggerer, logger *slog.Logger) *Manager {
	...
	m := &Manager{store: store, catalog: catalog, credentials: credentials, runner: runner, logger: logger,
		...
		unresolved: make(map[string]bool), configApps: make(map[string]map[string]bool),
...
func (m *Manager) RestartApp(appID string) {
	...
	if worker == nil {
		if m.appReferencesLocked(appID) > 0 || m.configReferencedLocked(appID) {
	...
	if m.workers[appID] == nil && (m.appReferencesLocked(appID) > 0 || m.configReferencedLocked(appID)) {
		m.startWorkerLocked(appID)
	}
	m.mu.Unlock()
}
...
func (m *Manager) stopUnused(appID string) {
	m.mu.Lock()
	worker := m.workers[appID]
	if worker == nil || m.appReferencesLocked(appID) > 0 || m.configReferencedLocked(appID) {
		m.mu.Unlock()
		return
	}
	worker.cancel()
...
func (m *Manager) emitStatusLocked(status ConnectionStatus) {
	...
	fn := m.onStatus
	if m.logger != nil {
		// Log only on a meaningful state transition; a pure counter bump
		// (envelopes/accepted/ignored/delivered counts, connected/envelope
		// timestamps) is not logged at all (mitto-qba) so a busy workspace
		// doesn't flood the console with tens of thousands of
		// identical-state lines per session — one per socket frame/envelope.
		...
		if meaningful {
			m.logger.Info("slackbridge: connection state changed",
				"app_id", status.AppID, "state", status.State, "subscription_count", status.SubscriptionCount,
