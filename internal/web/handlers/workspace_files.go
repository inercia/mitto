...
func (h *Handlers) HandleWorkspaceFiles(w http.ResponseWriter, r *http.Request) {
	...
	// like "apps/foo/*.md" resolves under apps/foo/ instead of silently
	// returning nothing (the non-recursive branch below matches base names, so
	// a multi-segment pattern would otherwise never match). The doublestar
	// branch already performs an equivalent hoist inside itself; doing it
	// once up-front here keeps both branches consistent and lets pure-literal
