...
func (h *Handlers) handleSetLoop(w http.ResponseWriter, r *http.Request, sessionID string, ps *session.LoopStore) {
	...
	if req.NoProgressLimit != nil {
		v := *req.NoProgressLimit
		p.NoProgressLimit = &v
	}
...
func (h *Handlers) handlePatchLoop(w http.ResponseWriter, r *http.Request, sessionID string, ps *session.LoopStore) {
	...
	if err := ps.Update(req.Prompt, req.PromptName, req.Frequency, req.Enabled, req.FreshContext, req.MaxIterations, req.Trigger, req.DelaySeconds, req.MaxDurationSeconds, req.Arguments, req.Condition, req.ConditionPreset, req.CooldownSeconds, req.CoalesceDuringBusy, req.NoProgressLimit); err != nil {
