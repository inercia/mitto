...
// writeCallbackError writes a flat {"error":code,"message":msg} JSON error.
// The callback endpoint is external-stable (see rest-api-conventions.md §6):
// external webhook callers depend on this legacy flat shape, so it intentionally
// does NOT use the canonical nested error envelope emitted by writeErrorJSON.
func writeCallbackError(w http.ResponseWriter, status int, errorCode, message string) {
...
func (h *Handlers) HandleCallbackTrigger(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method != http.MethodPost {
		writeCallbackError(w, http.StatusMethodNotAllowed, "method_not_allowed", "Only POST is supported")
		return
	}
	...
	if token == "" {
		writeCallbackError(w, http.StatusBadRequest, "missing_token", "Callback token is required")
		return
	}

	// 3. Validate token format
	if !session.ValidateCallbackToken(token) {
		writeCallbackError(w, http.StatusBadRequest, "invalid_token", "Invalid callback token format")
		return
	}

	// 4. Lookup session ID from index
	if h.deps.CallbackIndex == nil {
		writeCallbackError(w, http.StatusInternalServerError, "internal", "Callback index not available")
		return
	}
	sessionID, ok := h.deps.CallbackIndex.Lookup(token)
	if !ok {
		writeCallbackError(w, http.StatusNotFound, "not_found", "Callback not found")
		return
	}

	// 5. Check rate limit
	if h.deps.CallbackRateLimiter != nil && !h.deps.CallbackRateLimiter.Allow(token) {
		writeCallbackError(w, http.StatusTooManyRequests, "rate_limited", "Too many requests")
		return
	}
	...
	store := h.deps.Store
	if store == nil {
		writeCallbackError(w, http.StatusInternalServerError, "internal", "Session store not available")
		return
	}
	...
	if _, err := cs.Get(); err != nil {
		...
		writeCallbackError(w, http.StatusInternalServerError, "internal", "Failed to get callback config")
		return
	}
	...
	if err := h.deps.TriggerPeriodicNow(sessionID, true); err != nil {
		...
		default:
			...
			writeCallbackError(w, http.StatusInternalServerError, "internal", "Failed to trigger prompt")
		}
		return
	}
