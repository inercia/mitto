...
type permissionModal struct {
	sess    *client.Session
	pending []client.Event // queued permission events, front is the one shown
	styles  *styles
	width   int
}

func newPermissionModal(sess *client.Session, styles *styles) *permissionModal {
...
func (p *permissionModal) Push(ev client.Event) bool {
...
func (p *permissionModal) current() client.Event {
	if len(p.pending) == 0 {
		return client.Event{}
	}
	return p.pending[0]
