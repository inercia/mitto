...
func parseWorkspaceRC(data []byte) (*WorkspaceRC, error) {
	...
	for _, p := range raw.Prompts {
		...
		// "text" before validation, so a .mittorc inline prompt forward-migrated
		// to a newer parameter type still loads (as text) instead of being
		// silently dropped (mitto-f8l).
		if warnings := DegradeUnknownParameterTypes(p.Parameters); len(warnings) > 0 {
			for _, w := range warnings {
				slog.Warn("prompt parameter has unrecognised type; degraded to text", "prompt", p.Name, "source", ".mittorc", "detail", w)
			}
		}
