	...
	if scheduledTime == nil {
		if s.sessionManager != nil {
			...
			if bs == nil && !targetMeta.Archived {
				...
				if resumeErr != nil {
					// mitto-54k.6 / mitto-nf6: a cold-start MCP-init timeout or
					// shared-process saturation is transient (warm-once barrier
					// mitto-54k.3 will let a later resume succeed). Do not log the
					// hard "Failed to auto-resume stored session" warning; schedule
					// the shared bounded background retry (tools_children.go) so the
					// queued prompt is delivered once the process warms.
					if s.isTransientChildResumeError(resumeErr) {
						s.logger.Info("Auto-resume deferred: transient cold-start timeout; scheduling bounded retry",
							"target_session", input.ConversationID,
							"error", resumeErr)
						s.scheduleBoundedResumeRetry(input.ConversationID, targetMeta.Name, targetMeta.WorkingDir)
					} else {
