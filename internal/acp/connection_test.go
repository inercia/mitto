...
func TestNewConnection_EmptyCommand(t *testing.T) {
	...
	_, err := NewConnection(ctx, "", "", nil, true, nil, nil, nil)
	if err == nil {
...
func TestNewConnection_InvalidCommand(t *testing.T) {
	ctx := context.Background()
	_, err := NewConnection(ctx, "/nonexistent/command/that/does/not/exist", "", nil, true, nil, nil, nil)
	if err == nil {
...
func TestNewConnection_WithMockACP(t *testing.T) {
	...
	conn, err := NewConnection(ctx, mockPath, "", nil, true, func(msg string) {
...
func TestConnection_HasImageSupport_BeforeInitialize(t *testing.T) {
	...
	conn, err := NewConnection(ctx, mockPath, "", nil, true, nil, nil, nil)
	if err != nil {
		t.Fatalf("NewConnection failed: %v", err)
	}
	defer conn.Close()
...
func TestNewConnection_ShellQuotingErrors(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			ctx := context.Background()
			_, err := NewConnection(ctx, tc.command, "", nil, true, nil, nil, nil)
...
func TestNewConnection_ShellQuotingSuccess(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			ctx := context.Background()
			conn, err := NewConnection(ctx, tc.command, "", nil, true, nil, nil, nil)
			if err != nil {
