...
/**
 * Loop Prompt Pill tests.
 *
 * These tests verify the end-to-end flow of loop prompt triggering and
 * the NamedPromptPill appearing in the UI.
 *
 * Bug context:
 *   Before the fix, `checkAndFillGap` was called AFTER `updateLastKnownSeq`
 *   in the WebSocket message handlers. This meant that when a loop prompt
 ...
 *
 * Regression test:
 *   The pill should appear QUICKLY (within ~10s) after the loop prompt is
...
test.describe("Loop Prompt Pill", () => {
  test.beforeEach(async ({ page }) => {
    // Navigate to the app. We do NOT use navigateAndEnsureSession here because
    // after test 1 creates a loop session the app may auto-select it, and
    // loop sessions hide the textarea that waitForAppReady expects.
  ...
  test("should show NamedPromptPill when loop prompt is triggered via run-now", async ({
    page,
    request,
    helpers,
    timeouts,
    ...
    // 2. Configure loop prompt using the "Hello Greeting" prompt that exists in
    ...
    // 4. Trigger the loop prompt immediately via run-now (bypass the schedule).
    ...
    console.log(
      `[loop-prompt-pill] NamedPromptPill appeared after ${elapsed}ms ` +
