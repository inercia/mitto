#!/usr/bin/env bash
# Install an MCP server for Mistral Vibe
...
case "$SCOPE" in
    project)
        ...
        CONFIG_DIR="${WORKSPACE_PATH}/.mistral-vibe"
        CONFIG_FILE="${WORKSPACE_PATH}/.mistral-vibe/settings.json"
        ;;
    *)
        CONFIG_DIR="${HOME}/.mistral-vibe"
        CONFIG_FILE="${HOME}/.mistral-vibe/settings.json"
        ;;
esac
