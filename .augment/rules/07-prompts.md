...
| `slackChannel` | A Slack channel picker (mirrors the picker used in Loop settings): pick a Slack workspace, then type/search a channel. Expands to the **channel ID string** (drop-in replacement for a bare `type: text` channel-ID param). The installation is picker-local UI context and is never saved as a companion argument. Interactive, dialog-collected (never gates menu visibility). |
...
**`beadsList` exception**: `fetchBeadsListPromptsForWorkspace` in `web/static/hooks/useBeadsIntegration.js` deliberately does NOT apply `menuSatisfies` for the `beadsList` menu — the shortcut resolver would otherwise hide any prompt whose params the menu can't auto-supply (any plain `type: text` not marked `required: false`), rendering the shortcut button as disabled "Prompt … not found". The dispatch path `handleRunBeadsListPrompt` already opens the parameter dialog via `shouldOpenPromptDialog(prompt, "beadsList")` for anything it can't auto-fill, so the resolver only needs `promptMenuIncludes(p, "beadsList")` plus the backend's `enabledWhen`. Original bug: the builtin `Loop processing tasks` shortcut with a plain `FromBranch: type text, default "main"` param.
...
**Interactive picker parameters** (`isInteractivePickerParam` in `web/static/utils/prompts.js`: `boolean`, `prompts`, `filename`, `dirname`, `workspaceFolder`, `slackChannel`, and `text` **with a non-empty `options` list**) are all dialog-collected and never gate menu visibility — no menu context can auto-supply them. The dialog offers each picker unconditionally; a `filename`/`dirname` param whose backing folder is empty falls back to a text input so the user can still type a path.

**Backend/frontend registry mirror invariant**: `KnownPromptPrompParameterTypes` in `internal/prompts/param_types.go` and `KNOWN_PARAM_TYPES` in `web/static/utils/prompts.js` MUST stay in lockstep — any new parameter type added to the backend registry MUST also be added to the frontend `KNOWN_PARAM_TYPES` list, and (if interactive) to `isInteractivePickerParam`. A backend-only addition renders the type unknown to the dialog: the picker UI never appears, values are treated as generic strings, and menu-gating semantics silently diverge. Canonical user-facing catalog: `docs/config/prompts.md` (NOT `docs/devel/prompts.md`, which covers menu/dispatch architecture instead).

**Unknown parameter `type:` degrades to `text` (forward-compat, mitto-f8l)**: a parameter whose `type` is non-empty but not in the registry does NOT hard-fail prompt loading — `DegradeUnknownParameterTypes` (`internal/prompts/param_types.go`) rewrites it in place to the catch-all `text` and logs a `WARN` per degraded param (`parameter "X": unknown type "Y" degraded to "text"`). This lets an **older** running binary still load a builtin that a **newer** build forward-migrated to a type it predates (e.g. `slackChannel`), instead of rejecting the prompt and — as a downstream casualty — auto-pausing any loop whose named prompt could no longer be resolved. Wired into all three load paths: file builtins (`parsePromptFileData` → `prompt.Warnings`), top-level inline prompts (`internal/config/config.go`), and `.mittorc` inline prompts (`internal/config/workspace_rc.go`), aliased via `internal/config/prompts_shim.go`. Only the `type` axis degrades — other enums (`remember`, `show`, `options`, …) stay fail-fast (mitto-x8v). An **empty** `type` is left untouched (authoring mistake, not forward-migration) and `ValidatePromptParameters` still rejects it. Mirrors the `menus`-token warning precedent (`WarnUnknownMenus`, `internal/prompts/menus.go`): a lint-class field must not evict an otherwise-working prompt.
...
- **Beads** — aggregate `BeadsCount` / `HasBeads` covered in the paragraph below; single-bead helpers here: `BeadHasLabels(id, labels)` (ALL labels match), `BeadHasStatus(id, statuses)` (ANY status matches), `BeadIsOpen(id)`, `BeadMetadata(id, key)`. All fail-open.
...
Every **Tasks** prompt exposed through `menus: beadsIssues` also excludes support tickets in both menu-evaluation contexts: the selected row's `Item.Labels` and a conversation's linked bead. `TestTasksBeadsIssuePromptsHiddenForSupportIssues` inventories the builtin corpus so future Tasks prompts cannot omit either guard.
