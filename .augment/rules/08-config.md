...
`ACPServer.Constraints`: auto-select config options (model, etc.) on session start. MatchModes: `"contains"`, `"exact"`, `"startsWith"`, `"regex"`, `"lookAlike"` (word-based). **Model settings seed a conversation once**, behind an explicit initial-model preference. An existing `BaselineModel` always wins on resume/restart (mitto-1yo); `applyConfigConstraints()` reapplies that conversation model, not the workspace default. Initialization resolves and persists the intended model before the single startup RPC worker/barrier. Never persist the agent default first and then use metadata presence to detect a resumed session: that suppresses initial preferences on fresh conversations.
...
**Turn-scoped model lifecycle (mitto-1yo)**: reserve before rendering; await bounded model selection (never let a detached switch land during a later turn). Restore baseline and drain pending manual choices before releasing admission, including error/cancel/reset paths. Recheck pending selections under `promptMu` before becoming idle. Temporary changes never persist a baseline. Use `AgentModels()` snapshots, not raw shared pointers. A fresh ACP session must adopt its new ID/catalog and reapply the existing baseline before prompting.

**Tag-based resolution is a "tier gate" (intentional)**: `SelectPreferredModel` (`internal/conversation/constraints.go`) checks the CURRENT model FIRST via `currentSatisfiesProfile`. If the current model already matches any profile carrying the requested tag, it is kept and NO `SetSessionModel` RPC is issued (avoids needless set_model at wakeup — mitto-ykb). Same semantics apply when `cbInitBaselineModelIfEmpty` (`bgsession_callbacks.go`) seeds the initial-model tier chain `WorkspaceSettings > ACPServer settings > Agent default`. Consequence: if the default (e.g. Claude Opus 4.7) already carries the tag (`Smartest`, `Reasoning`, `Expensive` via the built-in `Claude Opus` profile), a Workspace/Settings `modelTag: Smartest` will NOT switch to a newer variant like Opus 4.8. **To force a specific model on new-session start, reference it by `modelName:` (exact profile), not `modelTag:`**. Diagnostic checklist when a user reports "Mitto did not open on model X": (a) which tier holds the setting, (b) is it `modelName:` or `modelTag:`, (c) does the current default already satisfy the tag (tier-gate no-op), (d) does the matcher's pattern accidentally substring-match a broader set of models.
