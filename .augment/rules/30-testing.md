...
**Known mock-ACP limitation — no capture-group / echo-back templating** (mitto-3od.6): fixture responses in `tests/fixtures/responses/*.json` are static text; the matcher cannot echo runtime-generated values from the inbound prompt into the response. Any end-to-end path that requires the LLM to echo back a caller-generated token — notably the auxiliary-session processor completion protocol, where `internal/processors/apply.go::parseProcessorCompletion` matches on the exact runtime `dispatch_id` in `MITTO_PROCESSOR_COMPLETION {"dispatch_id":"<id>","save_count":N}` — cannot be exercised through the mock as-is. Until the mock gains capture-group substitution, prefer a unit test against `processors.Apply` with a stub `ProcessorRunner`, or a "prompt-contract" test that asserts the built-in YAML prompt literally instructs the LLM to echo the received `dispatch_id`.
...
- **Any per-frame ACP callback fires ONCE PER WebSocket frame of that type, not per API call.** Confirmed instances: `OnEventsLoaded` (mitto-nhq, commit 07053230) and `OnPromptComplete` (mitto-w0a, commit 0b61ae7a). Multiple frames per test are the norm — handshake auto-load + explicit `LoadEvents` + paginated `hasMore` each deliver their own `events_loaded` frame; two sequential prompts deliver two `prompt_complete` frames. `close(sig)` unconditionally in the callback double-closes and panics. Canonical safe pattern: guard with `sync.Once` — see `chatui_smoke_test.go` (`loadedOnce.Do(func(){ close(loaded) })`), `session_edge_cases_test.go` (`initialLoadOnce`/`syncOnce`/`agentMessageOnce`), and `sequence_contract_test.go:55/134/251/466` (4 `OnPromptComplete` sites). Reproducing double-delivery for `LoadEvents` requires SEQUENTIAL non-overlapping calls (~500ms apart) because `handleLoadEventsAsync` in `internal/web/session_ws.go` uses `loadEventsMu.TryLock()` to silently drop concurrent duplicates; for `OnPromptComplete`, two sequential prompts on one connection with an unguarded close callback panic deterministically.
- **JS hook modules that destructure `window.preact` at module scope are single-stub-only across the suite** (mitto-b1k, 2026-09-19). `web/static/hooks/*.js` typically does `const { useState, useEffect } = window.preact;` at module scope, so bindings are captured ONCE per Bun ES-module cache entry. If two `*.test.js` files each install their own `window.preact` stub and then `await import("hooks/useSessionsStore.js")`, Bun serves the FIRST file's cached module to the SECOND — silently. Symptom: each file passes solo, `bun test web/static` reports 30+ unrelated failures. Rule: cover component derivation logic by mirroring it against the pure (window-free) store module (e.g. `web/static/stores/sessionsStore.js`) rather than dynamic-importing the hook wrapper from a new test file. See bd memory `js-hooks-module-cache-hazard-window-preact-destructure`.
