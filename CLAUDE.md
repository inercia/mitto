...
**Protocol-neutral backend seam** (`mitto-lrt.4`, additive): `internal/agentbackend/` defines neutral `Connection` / `ProviderDiscovery` / `SessionOps` / `EventDelivery` / optional `ClientServices` contracts + an in-memory `FakeHost` — the seam a second upstream protocol (e.g. AHP) plugs into. `internal/agentbackend/imports_test.go` transitively forbids `acp-go-sdk`, `internal/acp`, `internal/acpproc`, `internal/web`, `internal/conversation`, and `os/exec`. Do NOT relax the forbidden list to fix a build. **Legacy→neutral bridge** (`mitto-lrt.5`, additive): `internal/backendcompat/` maps `config.ACPServer`/`session.Metadata` → neutral types via dependency inversion (keeps `agentbackend` a pure leaf) with **byte-identical persistence** — nothing on disk changes; neutral views are computed on demand. `AgentRef` identity is `(Backend, Provider)` so display-name renames cannot rebind; `SessionRef` never collapses `SessionID` vs `ACPSessionID`; `CredentialRef` is reference-only; `ResolveProviderAlias` mirrors `migration_001_normalize_acp.go` semantics but rejects ambiguous canonical sets with `ErrAmbiguousAlias`. **ACP-to-neutral adapter** (`mitto-lrt.6`, additive): `internal/acpbackend/` wraps `conversation.SharedProcess` and implements all five `agentbackend` contracts via pure translators (content/outcome/capabilities/state/errors) + ack-gated model/mode setters + inbound ACP→neutral events tagged `Origin=OriginLocal`. This package is the protocol-specific home for `acp-go-sdk` that `agentbackend`'s own import guard forbids `agentbackend` itself from depending on; it is NOT imported by `internal/conversation` in this increment — live wiring into `BackgroundSession` is deferred to `mitto-lrt.7`, and event-projection extraction (tool-call/plan payload modeling, sequence/streaming projection) to `mitto-lrt.8`. Documented shims to remove alongside that later work: synthesized `ConversationID` in `NewSession` (real Mitto IDs arrive with .7 wiring); dropped Audio/embedded-Resource content blocks and deferred non-message `SessionUpdate` kinds (modeled in .8). See `docs/devel/agent-backend-architecture.md` and `.augment/rules/01-go-conventions.md` → "Transitive Import Guards".
...
- **Deferred-shared session MUST recompute MCP servers list at RPC time (mitto-220, commit 049fc0f7)**: `prepareSharedACPSession` used to compute the `mcpServers` list (including the mitto-apvg HTTP binding entry) ONCE at conversation-creation and stash it via `hsSetPendingSharedMcpServers`. Since `session/new` is deferred to the first prompt, a session prepared before the global MCP SSE listener or `McpCapabilities.Http` was ready shipped an empty list and was permanently stuck with `self_id="init"` correlation failures for its whole lifetime. Fix: `ensureSharedACPSession` (`internal/conversation/shared_session_handshaker.go`) now recomputes via `hsStartMcpServer` using `sharedProcess.Capabilities()` immediately before the deferred RPC. Safe because `RegisterSession` is idempotent (no token rotation on re-registration) — do NOT accept a change that makes `RegisterSession` non-idempotent. Generalized rule: any deferred-RPC parameter sensitive to system state that can evolve between prepare and dispatch (capabilities, port assignments, listener readiness) MUST be recomputed at dispatch time. See memory `deferred-shared-session-mcp-recompute-mitto-220`.
...
- **Rebuild-verification protocol (recurring trap)**: after asking the user to rebuild+restart, verify the fix is actually live **before** analyzing new logs. **macOS Make-target divergence (mitto-e9b, 2026-09-21)**: `make build` only rebuilds `./mitto` (CLI); the actually-running daemon is almost always `Mitto.app/Contents/MacOS/mitto-app` — the app bundle needs `make build-mac-app` + relaunch. Checks: (1) `ps -p <pid> -o comm=,lstart=` to identify which binary is running (don't assume `./mitto`); (2) `ls -la <that-binary>` mtime vs `git show -s --format=%ci <fix-commit>`; (3) `strings <that-binary> | grep <new-symbol>` — zero hits = not compiled in; (4) post-restart `grep <new-log-literal> mitto.log`. Watch for self-referential false positives — agent tool-call echoes containing the search literal get logged in the same session and can produce fake "N hits". Bitten at least 4× (mitto-54k.3/54k.5, mitto-xetv, mitto-mzvc, mitto-e9b). See `.augment/rules/40-debugging.md`.
...
**Silent-restore UI propagation is transport-fragile**: config option changes (including the silent baseline restore) reach the frontend ONLY via `GlobalEventsManager.Broadcast(WSMsgTypeConfigOptionChanged, …)` (`internal/web/events_ws.go:51` → `SendRaw`) — no seq, no ACK, not recorded in `events.jsonl`, no `load_events` replay. Slow-client backpressure closes the connection rather than buffering. The parallel per-session hook `SessionWSClient.OnConfigOptionChanged` (`session_ws.go:2952`) is **dead** server-side (`SessionObserver` has no `OnConfigOption*` method), so only the global path fires. **Frontend routing fix (mitto-6w3, commit 2e9911f2)**: `useWebSocket.js` now handles `config_option_changed` on the **global events socket** and forwards it to the per-session handler via `handleSessionMessageRef.current(msg.data.session_id, msg)` — the composer's `ConfigOptionSelect.js` selector follows the server for both explicit switches AND silent baseline restores without polling, reload, or a completion-time fetch. The remaining gap is only mid-reconnect: if the global-events socket is closed at broadcast time, the event is still lost and recovery falls back to the fresh `config_options` snapshot in the next `connected`/`acp_started` message. See `.augment/rules/22-web-frontend-websocket.md` "Global Events Broadcast: Fire-and-Forget, No Replay" and memory `config-option-changed-global-broadcast-no-replay-config`.
...
**Loop-supervisor idle-poll guard (mitto-d6h, commit 135472f1)**: `mitto_conversation_wait` has NO server-side per-turn call budget (`internal/mcpserver/tools_wait.go` `handleBeadsIssuesReachedState` blocks each call independently, no cross-call dedup) — a deviating supervisor agent can spin hundreds of waits in one turn to "poll deferred beads for eligibility" (observed 203 waits in one conversation before the fix). Any loop-supervisor prompt that yields on "nothing eligible" MUST carry an explicit anti-idle-poll paragraph forbidding both `mitto_conversation_wait`-for-eligibility AND intra-turn re-enumeration; the `beads_issues_reached_state` wait is sanctioned ONLY for beads THIS pass already dispatched (§B/§C), never as a discovery mechanism. Fix lives in `config/prompts/builtin/beads-issues/loop-processing.prompt.yaml` Step 6, pinned by `TestBeadsLoopPrompts_Defects_mittoD6h_NoIdlePollGuard`. Test lesson: literal-substring assertions on YAML prompt files require the required phrase to live on a single unbroken source line — folded-scalar wrapping is invisible to `strings.Contains`. See `.augment/rules/31-loop-prompts.md` § "Idle-Poll Guard".
