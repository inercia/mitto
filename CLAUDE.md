...
- **MCP `SessionTimeout` is POST-only (mitto-6cz6)**: `mcpStreamableHTTPOptions()` sets `SessionTimeout: 30 * time.Minute`, and the SDK only resets the idle timer on HTTP **POST** — SSE `GET` keepalive reconnects are invisible to it. That's what makes 30 min selective for ghost sessions (SSE-only) while sparing live ones (max observed live inter-POST gap ~10.5 min). Do **not** lower without re-checking gaps. **Compound upstream risk**: auggie NEVER sends `DELETE /mcp` (audit: `terminateSession` symbol has zero call sites in the shipped bundle — augmentcode/auggie#162) AND does not re-initialize on 404 (auggie#149). Today the 404 path is masked by **pooled-session failover** (subsequent `tools/call` transparently rides another live session in the same client's pool). If #162 is fixed upstream but #149 remains, the pool disappears and the 30-min timeout starts producing wedged tool calls — mitigation would be **raising** `mcpIdleSessionTimeout`, not removing it. Re-verify after any auggie upgrade. See `.augment/rules/42-mcpserver-development.md`.
