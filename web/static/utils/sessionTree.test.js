...
describe("sessionTree", () => {
  ...
  describe("getAllDescendants", () => {
    ...
    test("returns direct children", () => {
      const childrenMap = new Map([
        ["parent-1", [{ session_id: "child-1" }, { session_id: "child-2" }]],
      ]);
  ...
  describe("hasChildren", () => {
    ...
    test("returns true for session with children", () => {
      const childrenMap = new Map([["parent-1", [{ session_id: "child-1" }]]]);
  ...
  describe("detectCircularReference", () => {
    test("returns false for valid parent-child relationship", () => {
      ...
      expect(detectCircularReference("child-2", "parent-1", sessions)).toBe(
        false,
      );
    });
    ...
    test("detects direct self-reference", () => {
      const sessions = [];
      expect(detectCircularReference("session-1", "session-1", sessions)).toBe(
        true,
      );
    });
    ...
    test("detects circular reference in chain", () => {
      ...
      expect(detectCircularReference("child-1", "parent-1", sessions)).toBe(
        true,
      );
    });
  });
  ...
  describe("getSessionDepth", () => {
    test("returns 0 for root session", () => {
      const sessions = [{ session_id: "root-1", parent_session_id: "" }];
