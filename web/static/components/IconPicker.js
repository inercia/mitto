  ...
  return html`
    <div class="dropdown">
      <div
        ...
        class="btn btn-ghost btn-square btn-sm ${className} ${disabled
          ? "btn-disabled"
          : ""}"
      >
      ...
      <div
        tabindex="0"
        class="dropdown-content z-50 p-2 w-64 bg-base-200 rounded-box shadow-xl"
        role="listbox"
        aria-label="Available icons"
      >
        <!-- The flex grid lives on an INNER wrapper, never on .dropdown-content
             itself: a display utility (flex) placed directly on
             .dropdown-content lands in Tailwind's utilities layer and would
             override daisyUI's display:none collapse rule, leaving the grid
             permanently laid out (invisible but click-intercepting). -->
        <div class="flex flex-wrap gap-1">
          ...
          <button
            ...
            class="btn btn-ghost btn-square btn-sm border-2 border-dashed border-mitto-accent text-mitto-accent ${!hasIcon
              ? "bg-base-300 ring-1 ring-mitto-accent"
              : ""}"
          >
            <span class="w-4 h-4">
          ...
          ${iconNames.map((name) => {
            ...
            return html`
              <button
                ...
                class="btn btn-ghost btn-square btn-sm ${isSelected
                  ? "bg-base-300"
                  : ""}"
              >
                <span class="w-4 h-4">
