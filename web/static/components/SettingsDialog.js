...
function PromptEditForm({ prompt, onSave, onCancel, readOnly = false }) {
    ...
    const [backgroundColor, setBackgroundColor] = useState(prompt.backgroundColor || '');
        ...
        <div class="space-y-3">
            <div>
                ...
                <input
                    ...
                    onInput=${e => setName(e.target.value)}
                    disabled=${readOnly}
                    class="w-full px-3 py-2 bg-slate-700 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 ${readOnly ? 'opacity-60 cursor-not-allowed' : ''}"
                />
            </div>
            <div>
                ...
                <textarea
                    ...
                    rows="3"
                    disabled=${readOnly}
                    class="w-full px-3 py-2 bg-slate-700 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none ${readOnly ? 'opacity-60 cursor-not-allowed' : ''}"
                />
            </div>
            <div>
                ...
                <div class="flex items-center gap-2">
                    <input
                        ...
                        disabled=${readOnly}
                        class="w-10 h-10 rounded cursor-pointer border border-slate-600 ${readOnly ? 'opacity-60 cursor-not-allowed' : ''}"
                    ...
                    <input
                        ...
                        placeholder="#E8F5E9"
                        disabled=${readOnly}
                        class="flex-1 px-3 py-2 bg-slate-700 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 font-mono ${readOnly ? 'opacity-60 cursor-not-allowed' : ''}"
                    />
                    ${backgroundColor && !readOnly && html`
                        <button
                            type="button"
 ...
 */
export function SettingsDialog({ isOpen, onClose, onSave, forceOpen = false, WorkspaceBadge }) {
    ...
    const updateServer = (oldName, newName, newCommand, serverPrompts = []) => {
        ...
        setAcpServers(acpServers.map(s =>
            s.name === oldName ? { name: newName.trim(), command: newCommand.trim(), prompts: serverPrompts } : s
        ));
                                        ...
                                        ${acpServers.map(srv => html`
                                            <div key=${srv.name} class="p-3 bg-slate-700/20 rounded-lg border border-slate-600/50 hover:bg-slate-700/30 transition-colors group">
                                                ${editingServer === srv.name ? html`
                                                    <${ServerEditForm}
                                                        server=${srv}
                                                        onSave=${(name, cmd, serverPrompts) => updateServer(srv.name, name, cmd, serverPrompts)}
                                    ...
                                    <div class="p-4 bg-slate-800/50 rounded-lg border border-slate-700 space-y-3">
                                        ...
                                        <div class="flex justify-end gap-2">
                                            ...
                                            <button
                                                onClick=${() => {
                                                    if (newPromptName.trim() && newPromptText.trim()) {
                                                        const newPrompt = { name: newPromptName.trim(), prompt: newPromptText.trim() };
                                                        if (newPromptColor) {
                                            ...
                                            ${editingPrompt === index ? html`
                                                <${PromptEditForm}
                                                    ...
                                                    onSave=${(name, text, bgColor) => {
                                                        ...
                                                        if (bgColor) {
                                                            updated[index].backgroundColor = bgColor;
                                                        }
                                                        setPrompts(updated);
                                                    ...
                                                    <div class="flex items-center gap-2 flex-1 min-w-0">
                                                        ${prompt.backgroundColor && html`
                                                            <div
                                                                ...
                                                                style=${{ backgroundColor: prompt.backgroundColor }}
                                    ...
                                    ${authEnabled && html`
                                        <div class="p-4 bg-slate-700/20 rounded-lg border border-slate-600/50 space-y-3">
                                            ...
                                            <div class="flex items-center gap-2">
                                                ...
                                                />
                                                <span class="text-xs text-gray-500">(leave empty for random)</span>
                                            </div>
                                            <!-- Status indicator -->
                                            ${externalEnabled && currentExternalPort && html`
                                        ...
                                        <!-- Lifecycle Hooks -->
                                        <div class="p-4 bg-slate-700/20 rounded-lg border border-slate-600/50 space-y-3">
                                            <h5 class="text-sm font-medium text-gray-300">Lifecycle Hooks</h5>
                                            <p class="text-xs text-gray-500">Commands to run when external access starts/stops (e.g., for tunneling)</p>
                                            <div class="flex items-center gap-2">
                                            ...
                                            <div class="flex items-center gap-2">
                                                <label class="text-sm text-gray-400 w-12">Down</label>
                                                <input
                                                    type="text"
                            ...
                            <div class="space-y-4">
                                ...
                                <div class="space-y-3">
                                    ...
                                    <label class="flex items-center gap-3 p-3 bg-slate-700/20 rounded-lg border border-slate-600/50 cursor-pointer hover:bg-slate-700/30 transition-colors">
                                        ...
                                        />
                                        <div>
                                            <div class="font-medium text-sm">Confirm before deleting conversations</div>
                                            <div class="text-xs text-gray-500">Show a confirmation dialog when deleting a conversation</div>
                                        </div>
                                    </label>
                                </div>
                                ...
                                ${isMacApp && html`
                                    <div class="space-y-3">
                                        <h4 class="text-sm font-medium text-gray-300">macOS Settings</h4>
                                        <label class="flex items-center gap-3 p-3 bg-slate-700/20 rounded-lg border border-slate-600/50 cursor-pointer hover:bg-slate-700/30 transition-colors">
                                            <input
                                                type="checkbox"
                                            ...
                                            />
                                            <div>
                                                <div class="font-medium text-sm">Play sound when agent completes</div>
                                                <div class="text-xs text-gray-500">Play a notification sound when the AI finishes responding</div>
                                            </div>
                                        </label>
                                        <label class="flex items-center gap-3 p-3 bg-slate-700/20 rounded-lg border border-slate-600/50 cursor-pointer hover:bg-slate-700/30 transition-colors">
                                            <input
                                            ...
                                            <div>
                                                ...
                                                <div class="text-xs text-gray-500">Make the window visible in all macOS Spaces (requires restart)</div>
                                            </div>
                                        </label>
                                    </div>
                                `}
