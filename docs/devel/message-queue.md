...
- **Layer 0.5 — MCP-init readiness gate (mitto-tgx).** Before any fire attempt resolves a prompt, `triggerTasksFireWithRetry` checks `BackgroundSession.IsMCPInitGated` — true when the session's shared ACP process has either reported its MCP-init wait timed out, or has a cold-start handshake still in progress that has not completed (`timedOut || (inProgress && !done)`, the same predicate the auxiliary-session bail uses in `internal/acpproc`, mitto-337). While gated, the fire is **deferred** (delta preserved, quiescence timer re-armed) and, critically, does **not** increment `tasksRefireDeliveryFailures` — a cold-starting agent is not a genuine delivery failure, so a slow MCP init can no longer burn through the `maxTasksRefireDeliveryFailures` ceiling with doomed `failed to clear context` prompt-prep attempts. The boot pulse (`fireOnStartPulses`) applies the same gate and rolls back its once-per-process `runOnStartFired` guard so it retries once un-gated. The gate fails open (treated as not gated) if the underlying process does not expose the MCP-init signals. Note this threshold is deliberately lower than the GC "wedged" 2×-timeout recycle bar: deferring a benign loop fire is cheap, whereas recycling a process is destructive.
