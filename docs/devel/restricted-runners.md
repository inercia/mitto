...
Paths that resolve to an empty string are dropped by `ResolvePaths`, so an unset
variable never widens the sandbox to every path via an empty allow-list entry.

### Automatic write-folder safety nets

For restricted runners only, `NewRunner` always ensures the agent's own working
directory (`$MITTO_WORKING_DIR`) is present in `allow_write_folders`
(deduplicated).
