...
| —                                                        | `Workspace.MissingUserDataFieldCount`                       | `Workspace.MissingUserDataFieldCount` — number of `.mittorc` `user_data` schema fields whose value is unset or blank on this conversation; `0` when no schema is defined (mitto-685)                                                                                                                                          |
| —                                                        | `Workspace.AllUserDataResolved`                             | `Workspace.AllUserDataResolved` — `HasUserDataSchema && MissingUserDataFieldCount == 0`; fail-closed to `false` when no schema is defined so a schemaless workspace never looks "resolved" (mitto-685)                                                                                                                        |
...
| `{{ .Prompts.Enabled "name" }}`                          | `Prompts.IsEnabled("name")` / `"name" in Prompts.EnabledNames` | `Prompts.Enabled(name string) bool` — case-insensitive check for a currently-enabled prompt. Disabled prompts are pruned from the cache during reload, so `Enabled` and `Exists` share the same set (any name that resolves via `mitto_prompt_get` returns `true`). Empty name and cold-start fail-closed. CEL predicate `Prompts.IsEnabled(name)` (mitto-3od.1) is a receiver macro over `Prompts.EnabledNames` and delegates to the same Go method, so template and CEL views agree byte-for-byte. |
...
**Prompt registry (mitto-3od.1):** `Prompts.EnabledNames` (declared as `cel.Variable("Prompts.EnabledNames", cel.ListType(cel.StringType))` in `NewCELEvaluator`) exposes the effective enabled-prompt list to `enabledWhen`. The receiver macro `Prompts.IsEnabled(name)` rewrites to `__mitto_promptEnabled(Prompts.EnabledNames, name)` which delegates to the same `PromptsContext.Enabled` Go method backing the `.Prompts.Enabled` template accessor, so both views agree byte-for-byte. Membership is case-insensitive; an unknown/cold-start snapshot (nil slice) fails closed. Wiring lives in `internal/cel/evaluator.go` (`promptsIsEnabledMacro`, `mittoPromptEnabled`) with the activation mapping `"Prompts.EnabledNames": ctx.Prompts.EnabledNames`. Primary use case: a legacy processor or prompt disabling itself once a consolidating peer is registered (`enabledWhen: '!Prompts.IsEnabled("close-phase knowledge router")'`).
