...
| `slackChannel`    | A Slack channel picker (mirrors the picker used in Loop settings): pick a Slack workspace (installation), then either type a channel ID or search for a channel. The value expanded into the template is the **channel ID string** (a drop-in replacement for a bare `type: text` channel-ID param). The installation is picker-local UI context only and is never saved as a companion argument. Interactive and dialog-collected (never gates menu visibility, always offered by the parameter dialog).                                                                                                                                                                                                                                                                                           |
