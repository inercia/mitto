...
| `beads-track-tasks`           | Reminds the agent to track tasks and knowledge in beads (`bd`) instead of markdown TODO lists. Shrinks to the runtime-resolved `BeadsDatabaseMode` line when a standing agent-instructions file already covers the rest (`HasStandingBeadsGuidance()`, mitto-2kw). | userPrompt / first       | text    | Yes (requires `bd` command on PATH)                                             |
| `beads-ready-tasks`           | Reminds the agent to review available tasks (`bd ready`) when a beads database exists. Fully suppressed when a standing agent-instructions file already carries the same guidance (`HasStandingBeadsGuidance()`, mitto-2kw). | userPrompt / first       | text    | Yes (requires `bd` command + `.beads` directory + no standing Beads guidance)   |
...
| `identify-user-data`          | Detects user data values from conversations and sets them via MCP (every 2 turns or 6k tokens)            | agentResponded / all     | prompt  | **Yes** (only activates when `user_data` schema is defined in `.mittorc` **and** at least one schema field is still unresolved on the conversation — `Workspace.MissingUserDataFieldCount > 0`, mitto-685) |
...
parameters:
  - name: HistoryLimit # placeholder name → ${HistoryLimit}
    type:
      text # one of: beadsId beadsTitle sessionId childSessionId
...
when: # Trigger condition — always a block
  on:
    userPrompt # Phase: "userPrompt" (before send), "agentResponded" (after each turn),
...
output:
  transform # "transform", "prepend", "append", "discard" (default: transform)
...
- `first` — fires only on the _first-ever_ message in the conversation. This state **persists across session restarts** — if the processor already fired before the session was stopped and resumed, it will not fire again.
- `all` — fires on every message.
- `allExceptFirst` — fires on every message _except_ the first. _(Note: camelCase only — `all-except-first` is rejected.)_
...
- `Prompts.IsEnabled("prompt name")` — true if a workspace prompt with that name (case-insensitive) is registered and enabled; false if unknown, disabled, or the registry snapshot is unavailable (fails closed). Used by the builtin legacy memory/rules processors (`memorize-preferences`, `extract-memories-on-close`, `auggie-update-rules`, `claude-update-memory`) to suppress themselves once the `knowledge-router` close-phase processor is enabled: `enabledWhen: '!Session.IsLoop && !Prompts.IsEnabled("knowledge-router")'`. For `on: conversationClosed` processors this requires the workspace prompt registry to be wired through (`SessionManager.SetPromptsCache`) — if unwired, the predicate fails closed and any negated gate (`!Prompts.IsEnabled(...)`) resolves to `true`, i.e. the processor still runs.
...
- `HasStandingBeadsGuidance()` — true iff a canonical agent-instructions file at the workspace root (`AGENTS.md`, `AGENT.md`, `CLAUDE.md`, `CLAUDE.local.md`, `CONVENTIONS.md`, `GEMINI.md`) already carries the standing Beads workflow markers (`bd ready --exclude-label in-flight` AND `bd remember`, case-insensitive). Fail-open (missing / oversize > 256 KB / I/O error → `false`), memoised 2 s per folder. Used by the builtin `beads-track-tasks` and `beads-ready-tasks` processors to shrink or suppress themselves when the workspace has already committed the same guidance to a standing file (mitto-2kw).
