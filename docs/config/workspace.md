...
Settings shared by every ACP server entry in the same folder — the display `name`, badge `code`, badge `color`, the organizational `group` label, `auto_children`, the `pinned` flag (show the folder in the sidebar even when it has no conversations), and the Beads `beads` subsection — are stored once per folder in a `folders.json` file in the Mitto data directory (`$MITTO_DIR`), keyed by working directory. The folder `group` (e.g. "development", "personal", "operations") is Mitto-local and distinct from the `.mittorc` metadata `group`.
...
- **Database mode** — `local` keeps the Dolt database local to this machine; `shared` permits replication through a configured Dolt remote. The value is persisted as `beads.databaseMode` in `folders.json` and is independent of the upstream task system. For legacy folders without a value, Mitto infers `shared` when a Dolt remote exists, otherwise `local`, then persists that result.
